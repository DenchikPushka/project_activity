...
class ModelTasks
{
	...
	public function addNewTask($project_id, $name, $description, $data) {
		...
		if (!empty($values)) {
			$mysqli->real_query("
				INSERT INTO `tasks_map` (`group_id`, `task_id`)
				VALUES $values
			");
		} else {
	...
	public function getTasksByKid($proj_id, $user_id) {
		...
		$mysqli->real_query("
			SELECT 	`t`.`id`,
					...
					`t`.`closed`
			  FROM 	`tasks` AS `t`
					INNER JOIN `tasks_map` AS `tm`
						ON `t`.`id` = `tm`.`task_id`
					INNER JOIN `groups_map` AS `gm`
						ON `tm`.`group_id` = `gm`.`group_id`
			  WHERE `t`.`project_id` = $proj_id AND
			  		`gm`.`user_id` = $user_id
			  GROUP BY `t`.`id`");
		$result = $mysqli->loadObjectsList();
